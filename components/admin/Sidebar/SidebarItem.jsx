    ...
    <>
      <li>
        <Link
          href={item.route}
          onClick={handleClick}
          className={`${pageName === item.label.toLowerCase() ? "bg-primary/[.07] text-primary " : "text-dark-4 hover:text-[var(--primary )]"} group relative flex items-center gap-3 rounded-[7px] px-6 py-3 font-medium duration-300 ease-in-out`}
        >
          {item.icon}
          {item.label}
          {item.message && (
